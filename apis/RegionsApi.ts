...
export class RegionsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * List all Regions
     */
    public async regionList(_options?: Configuration): Promise<RequestContext> {
...
export class RegionsApiResponseProcessor {
     ...
     */
     public async regionList(response: ResponseContext): Promise<RegionList200Response > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: RegionList200Response = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "RegionList200Response", ""
            ) as RegionList200Response;
            return body;
        }
